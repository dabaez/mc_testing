...
def run_gnn_training(q_torch, dgl_graph, net, embed, optimizer, number_epochs, tol, patience, prob_threshold,runtime_limit):
    """
    ...
    for epoch in range(number_epochs):
        ...
        if time() - t_gnn_start >= runtime_limit:
            print(f'Stopping early on epoch {epoch} because of time limit')
            break
...
def MaxCut(nx_graph, runtime_limit, dim_embedding , hidden_dim , learning_rate , dropout , seed_value = 1):
    ...
    _, epoch, final_bitstring, best_bitstring = run_gnn_training(
        ...
        gnn_hypers['tolerance'], gnn_hypers['patience'], gnn_hypers['prob_threshold'],runtime_limit)
    ...
    return gnn_tot_time, float(size_mc)