    ...
    for size in t:
        for i in range(5):
            ...
            for (u,v,w) in nx_temp.edges(data=True):
                w['weight'] = random.randint(-99,100)
                if (w['weight'] <= 0):
                    w['weight'] -= 1
            ...
            with open(f"my_graphs/gb_m100_100i/gb_m100_100i_{size}_{density}_{i}", 'w') as file:
                
                file.write(f"# graph with density {density} created with networkx fast_gnp_random_graph with seed {seed} and edges weighted randomly from [-100,-1] and [1,100]\n")
